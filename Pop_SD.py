...
def get_output():
    ...
    # Reshaping the data
    values = data['Pop_SD'][1:50].values.reshape(-1, 1)
    ...
    # To display in GUI
    Ypred = round(next_prediction[0][0] / 457, 4)
    print(Ypred)

    min_range = round((next_prediction[0][0] - std_dev) / 457 , 4)
    print(min_range)

    max_range = round((next_prediction[0][0] + std_dev) / 457 , 4)
    print(max_range)
